...
def show():
    ...
    dept_filter = st.sidebar.selectbox("Department", ["All"] + sorted(emp_df["Department"].dropna().unique()) if not emp_df.empty else ["All"])
    role_filter = st.sidebar.selectbox("Role", ["All"] + sorted(emp_df["Role"].dropna().unique()) if not emp_df.empty else ["All"])
    ...
    st.dataframe(
        ...
        if not filtered_df.empty else pd.DataFrame(columns=["Emp_ID","Name","Department","Role","Skills","Location"]),
        height=400
    )
        ...
        if st.button("Update Role"):
            if new_role != "Keep Current":
                try:
                    ...
                    # Use safe rerun instead of experimental
