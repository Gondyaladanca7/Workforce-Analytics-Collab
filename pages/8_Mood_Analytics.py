...
if notifications_df.empty:
    notifications_df = pd.DataFrame(columns=["emp_id","message","date"])
...
start_date = st.sidebar.date_input(
    "Start Date", value=mood_df["date"].min() if not mood_df.empty else datetime.date.today()
)
end_date = st.sidebar.date_input(
    "End Date", value=mood_df["date"].max() if not mood_df.empty else datetime.date.today()
)
...
if filtered_df.empty:
    st.warning("⚠️ No mood entries match the selected filters.")
...
# -----------------------
# Mood Distribution (Fixed)
# -----------------------
st.markdown("### 📊 Mood Distribution")
if not filtered_df.empty:
    mood_counts = filtered_df["mood"].value_counts()
    if not mood_counts.empty:
        dist_df = mood_counts.reset_index()
        dist_df.columns = ["Mood","Count"]  # FIXED column names for Plotly
        dist_fig = px.bar(
            dist_df,
            x="Mood",
            y="Count",
...
if not filtered_df.empty:
    box_fig = px.box(
        filtered_df, x="Employee", y=filtered_df["mood"].map(mood_score_map),
    ...
    with st.form("generate_master_pdf"):
        ...
        if submit_pdf:
            ...
            try:
                ...
                st.download_button(
                    "Download PDF", pdf_buffer, "workforce_master_report.pdf", "application/pdf"
                )
            except Exception as e:
